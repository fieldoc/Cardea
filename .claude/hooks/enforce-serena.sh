      ...
      cat <<EOF
{
  "hookSpecificOutput": {
    ...
    "permissionDecisionReason": "This looks like a code symbol search. Use Serena instead: mcp__plugin_serena_serena__find_symbol (for definitions) or mcp__plugin_serena_serena__find_referencing_symbols (for usages) or mcp__plugin_serena_serena__search_for_pattern (for text in code). See .claude/rules/serena.md for the full decision guide. IMPORTANT FALLBACK: if Serena tools are not available in this session (i.e. the tool returns 'tool not found' or similar), Grep is explicitly permitted — do not attempt Serena again, just proceed with Grep."
  }
}
EOF
      exit 0
