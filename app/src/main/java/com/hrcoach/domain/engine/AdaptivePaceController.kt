...
class AdaptivePaceController(
    ...
    private fun lookupPaceBias(paceMinPerKm: Float, targetHr: Int): Float {
        ...
        val totalWeight = neighbors.sumOf { it.sampleCount.toDouble() }.toFloat()
        val avgBucketHr = if (totalWeight > 0f) {
            neighbors.sumOf { (it.avgHr * it.sampleCount).toDouble() }.toFloat() / totalWeight
        } else {
            neighbors.sumOf { it.avgHr.toDouble() }.toFloat() / neighbors.size
        }
